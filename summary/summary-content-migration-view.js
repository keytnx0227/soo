...
export async function showSummaryContentMigrationReport({ migrated, failed }) {
    ...
    content.innerHTML = `
        ...
        <p class="stsm-migration-report-note">성공한 레코드는 기존 본문과 재렌더링 결과가 일치함을 확인했습니다. 제외된 레코드의 원문과 구조화 데이터는 변경하지 않았습니다.</p>
    `;
...
function renderSuccess(item) {
    const detail = item.changedFields.length
        ? item.changedFields.join(', ')
        : '구조화 데이터와 동일 · 편집 상태만 정리';
    return `
